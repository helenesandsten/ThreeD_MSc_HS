...
roots_plot1 <- ggplot(data = roots.df, 
                      aes(x = RIC_length_cm, y = root_mass_g), 
                      color = warming) + 
...
plot_roots_w <- roots.df %>% 
  filter(grazing == "C") %>% 
  filter(Nlevel == 1 & 2 & 3) %>% 
  ggplot(data = roots.df, mapping = aes(x = warming, y = root_mass_g)) +
...
## aud helps: move this to fixup script
roots.df <- roots.df %>% 
...
plot_roots_mega <- roots.df %>% 
  ...
  summarise(root_mass_g = mean(root_mass_g)) %>% 
  ggplot(mapping = aes(x = log(Namount_kg_ha_y +1), y = root_mass_g, color = warming)) +
  geom_point() +
  theme_bw(base_size = 20) +
  labs(title = "Effect of Nitrogen on root growth", 
       x = bquote(log(Nitrogen)~(kg~ha^-1~y^-1)), 
